...
# --- Turnaround entre trabajos en la MISMA posición (en días) ---
GAP_SAME = 0.08   # si es el mismo cliente
GAP_DIFF = 0.08   # si cambia de cliente
...
def read_input(xlsx_path: str, case_sheet: str, planning_start=PLANNING_START, planes_sheet: str = "Planes2"):
    ...
    try:
        if any(j.startswith("54-") for j in jobs):
            ...
            print(f"[DEBUG] BASE={base_date.date()}  ES(54)={es[j54]}  ES_date={(base_date + pd.to_timedelta(es[j54],'D')).date()}  "
...
def build_model(d,
    ...
    planes    = sorted(set(d['plane_of'][j] for j in jobs))
    clients   = sorted(set(d['client'][j]   for j in jobs))
    inter_in  = list(d.get('INTERF_IN', []))
    inter_out = list(d.get('INTERF_OUT', []))

    # Pares j<k (para no-solape y gaps)
    pair_list = [(jobs[i], jobs[j]) for i in range(len(jobs)) for j in range(i+1, len(jobs))]

    # Conjuntos
    m.J  = Set(initialize=jobs, ordered=True)
    m.P  = Set(initialize=positions, ordered=True)
    m.R  = Set(initialize=planes,  ordered=True)
    m.C  = Set(initialize=clients, ordered=True)
    m.JK = Set(initialize=pair_list, dimen=2, ordered=True)   # pares (j,k), j<k

    # Slots discretos para métricas
    S_min = max(0, int(math.floor(min(d['early'].values()))))
    S_max = int(math.ceil(max(d['late'].values())))
    if S_max <= S_min: S_max = S_min + 1
    m.S = RangeSet(S_min, S_max-1)

    # Parámetros
    m.ES = Param(m.J, initialize=d['early'])
    m.D  = Param(m.J, initialize=d['dur'])
    m.LF = Param(m.J, initialize=d['late'])
    m.H  = Param(initialize=float(d['H']))

    bigM = float(d['H'] + max(d['dur'].values()) + 100.0)
    m.bigM = Param(initialize=bigM, within=Reals)

    m.planeOf  = Param(m.J, initialize={j:d['plane_of'][j] for j in jobs}, within=Any)
    m.clientOf = Param(m.J, initialize={j:d['client'][j]   for j in jobs}, within=Any)

    # --- GAP por par (j,k): si cambia de cliente -> GAP_DIFF, si no -> GAP_SAME
    gap_init = {}
    for (j,k) in pair_list:
        gap_init[(j,k)] = GAP_DIFF if str(d['client'][j]) != str(d['client'][k]) else GAP_SAME
    m.GAPjk = Param(m.JK, initialize=gap_init, within=NonNegativeReals)

    # Variables principales
    m.t_start = Var(m.J, within=NonNegativeReals, bounds=(0, value(m.H)))
    m.t_end   = Var(m.J, within=NonNegativeReals, bounds=(0, value(m.H)))
    m.y       = Var(m.J, m.P, within=Binary)                    # asignación a posición
    m.t_in    = Var(m.J, m.P, within=NonNegativeReals, bounds=(0, value(m.H)))
    m.t_out   = Var(m.J, m.P, within=NonNegativeReals, bounds=(0, value(m.H)))

    # Asignación única + ventanas
    ...
    # Enlaces IN/OUT ↔ start/end (doble desigualdad para fijarlos cuando y[j,p]=1)
    m.c_in_lo  = Constraint(m.J, m.P, rule=lambda mdl,j,p: mdl.t_in[j,p]  >= mdl.t_start[j] - mdl.bigM*(1 - mdl.y[j,p]))
    m.c_in_hi  = Constraint(m.J, m.P, rule=lambda mdl,j,p: mdl.t_in[j,p]  <= mdl.t_start[j] + mdl.bigM*(1 - mdl.y[j,p]))
    m.c_out_lo = Constraint(m.J, m.P, rule=lambda mdl,j,p: mdl.t_out[j,p] >= mdl.t_end[j]   - mdl.bigM*(1 - mdl.y[j,p]))
    m.c_out_hi = Constraint(m.J, m.P, rule=lambda mdl,j,p: mdl.t_out[j,p] <= mdl.t_end[j]   + mdl.bigM*(1 - mdl.y[j,p]))

    # --- NO-SOLAPE EN POSICIÓN con turnaround basado en t_in/t_out ---
    # Z[j,k,p] = 1 ⇒ j va ANTES que k en la posición p
    m.Z = Var(m.JK, m.P, within=Binary)

    def no_overlap_pos_dir1(mdl, j, k, p):
        # k entra en p después de que j sale de p + GAPjk, si ambos usan p y Z=1
        return mdl.t_in[k,p] >= mdl.t_out[j,p] + mdl.GAPjk[j,k] \
               - mdl.bigM*(1 - mdl.y[j,p]) - mdl.bigM*(1 - mdl.y[k,p]) - mdl.bigM*(1 - mdl.Z[j,k,p])

    def no_overlap_pos_dir2(mdl, j, k, p):
        # j entra en p después de que k sale de p + GAPjk, si ambos usan p y Z=0
        return mdl.t_in[j,p] >= mdl.t_out[k,p] + mdl.GAPjk[j,k] \
               - mdl.bigM*(1 - mdl.y[j,p]) - mdl.bigM*(1 - mdl.y[k,p]) - mdl.bigM*(mdl.Z[j,k,p])

    m.c_nl1 = Constraint(m.JK, m.P, rule=no_overlap_pos_dir1)
    m.c_nl2 = Constraint(m.JK, m.P, rule=no_overlap_pos_dir2)

    # --- Interferencias IN/OUT entre posiciones (si las tienes cargadas) ---
    if len(inter_in) > 0:
        m.AB_in = Set(initialize=inter_in, dimen=2, ordered=True)
        m.Zin   = Var(m.JK, m.AB_in, within=Binary)
        def _cin1(mdl, j,k,a,b):
            # IN de k en b después de OUT de j en a
            return mdl.t_in[k,b] >= mdl.t_out[j,a] \
                   - mdl.bigM*(1 - mdl.y[j,a]) - mdl.bigM*(1 - mdl.y[k,b]) - mdl.bigM*(1 - mdl.Zin[j,k,a,b])
        def _cin2(mdl, j,k,a,b):
            # IN de j en a después de OUT de k en b
            return mdl.t_in[j,a] >= mdl.t_out[k,b] \
                   - mdl.bigM*(1 - mdl.y[j,a]) - mdl.bigM*(1 - mdl.y[k,b]) - mdl.bigM*(mdl.Zin[j,k,a,b])
        m.c_in1 = Constraint(m.JK, m.AB_in, rule=_cin1)
        m.c_in2 = Constraint(m.JK, m.AB_in, rule=_cin2)

    if len(inter_out) > 0:
        m.AB_out = Set(initialize=inter_out, dimen=2, ordered=True)
        m.Zout   = Var(m.JK, m.AB_out, within=Binary)
        def _cout1(mdl, j,k,a,b):
            return mdl.t_out[k,b] >= mdl.t_in[j,a] \
                   - mdl.bigM*(1 - mdl.y[j,a]) - mdl.bigM*(1 - mdl.y[k,b]) - mdl.bigM*(1 - mdl.Zout[j,k,a,b])
        def _cout2(mdl, j,k,a,b):
            return mdl.t_out[j,a] >= mdl.t_in[k,b] \
                   - mdl.bigM*(1 - mdl.y[j,a]) - mdl.bigM*(1 - mdl.y[k,b]) - mdl.bigM*(mdl.Zout[j,k,a,b])
        m.c_out1 = Constraint(m.JK, m.AB_out, rule=_cout1)
        m.c_out2 = Constraint(m.JK, m.AB_out, rule=_cout2)

    # --- Métricas/KPIs (como las tenías) ---
    m.vPresence       = Var(m.S, m.P, m.R, within=Binary)
    ...
    jobs_of_plane = {r:[j for j in jobs if d['plane_of'][j]==r] for r in planes}
    for r in planes:
        Js = jobs_of_plane[r]
        for p in m.P:
            for s in m.S:
    ...
    for p in m.P:
        for s in m.S:
            ...
            # con promedio sobre r para no sobredimensionar
    ...
    for c in m.C:
        for j in m.J:
            if str(d['client'][j]) == str(c):
                m.add_component(f"c_cli_delay_{c}_{j}",
                    Constraint(expr = m.vClientDelay[c] >= m.t_end[j] - m.LF[j]))

    # Cliente → Posición (soft), igual que ya tenías
    if client_pos_policy.lower() == "soft":
        m.z = Var(m.C, m.P, within=Binary)          # z[c,p] = 1 si el cliente c usa la posición p
        m.viol_C = Var(m.C, within=NonNegativeReals)

        def _link_y_to_z(mdl, j, p):
            c = mdl.clientOf[j]
            return mdl.y[j, p] <= mdl.z[c, p]
        m.c_link_yz = Constraint(m.J, m.P, rule=_link_y_to_z)

        m.c_client_onepos_soft = Constraint(
            m.C, rule=lambda mdl, c: sum(mdl.z[c, p] for p in mdl.P) <= 1 + mdl.viol_C[c]
        )
    else:
        m.z = None
        m.viol_C = None
    ...
    # Makespan opcional
    m.Tmax = Var(within=NonNegativeReals, bounds=(0, value(m.H)))
    m.c_mks = Constraint(m.J, rule=lambda mdl,j: m.Tmax >= mdl.t_end[j])

    # Objetivo (igual que antes + tus pesos)
    obj_terms = []
    ...
    if (client_pos_policy.lower() == "soft") and (m.viol_C is not None):
...
def generate_report(df_pos: pd.DataFrame, d: dict, movimientos=None):
    ...
    ES = d['early']; D = d['dur']; LF = d['late']; CL = d['client']; plane_of = d['plane_of']
    ESpl = d.get('ES_plane', {})
    LFpl = d.get('LF_plane', {})
    ...
    for _, r in df.sort_values(['plane','start']).iterrows():
        j = r['job']
        esj = ES.get(j, 0.0); dj = D.get(j, r['finish']-r['start']); lfj = LF.get(j, r['finish'])
        det.append({
            '✔': 'OK',
            'Avión': plane_of[j],
            'Trabajo': j,
...
def validate_solution(model, df_pos, INTERF_IN, INTERF_OUT):
    ...
    for p,grp in df_pos.groupby('p'):
        g=grp.sort_values('start'); prev=-1e-9
...
def solve_and_report(model, xlsx_path, case_sheet, timelimit=1500, mipgap=GAP):
    ...
    for j in model.J:
        ...
        st = float(value(model.t_start[j])); en = float(value(model.t_end[j]))
        tin = float(value(model.t_in[j,pos])); tout = float(value(model.t_out[j,pos]))
        ...
        movimientos.append((plane,pos,'IN',tin)); movimientos.append((plane,pos,'OUT',tout))
    ...
    ok=True
    for p,grp in df_pos.groupby('p'):
        g=grp.sort_values('start'); prev=-1e9
        for _,r in g.iterrows():
    ...
    kpis=[]
    for p,grp in df_pos.groupby('p'):
        kpis.append({'position':p,'jobs':len(grp),'start_min':grp['start'].min(),'finish_max':grp['finish'].max()})
...
def fact_check_solution(model, data, tol=1e-6, csv_prefix="violations"):
    ...
    for j in model.J:
        ES = float(value(model.ES[j])); D = float(value(model.D[j])); LF = float(value(model.LF[j]))
        ts = float(value(model.t_start[j])); te = float(value(model.t_end[j]))
        if ts < ES - tol:
            drv_rows.append({"check":"time_window_low","entity":str(j),"lhs":ts,"rel":">=","rhs":ES,"violation":ES-ts})
        if te > LF + tol:
            drv_rows.append({"check":"time_window_high","entity":str(j),"lhs":te,"rel":"<=","rhs":LF,"violation":te-LF})
        if abs((ts + D) - te) > tol:
            drv_rows.append({"check":"duration","entity":str(j),"lhs":ts + D,"rel":"==","rhs":te,"violation":abs((ts+D)-te)})
    ...
    if hasattr(model, "E_in") and hasattr(model, "E_out") and hasattr(model, "y"):
        ...
        for j in model.J:
            p = _p_of(j)
            ts = float(value(model.t_start[j])); te = float(value(model.t_end[j]))
            starts_by_pos[p].append((ts, j)); ends_by_pos[p].append((te, j))

        # IN: no pueden coincidir entradas en posiciones en conflicto
        for (a,b) in model.E_in:
            A = starts_by_pos.get(a, []); B = starts_by_pos.get(b, [])
            for tsA, jA in A:
                for tsB, jB in B:
        ...
        for (a,b) in model.E_out:
            A = ends_by_pos.get(a, []); B = ends_by_pos.get(b, [])
            for teA, jA in A:
                for teB, jB in B:
