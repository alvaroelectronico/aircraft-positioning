...
def ap_pyomo_model():
    ...
    model.vIdle = Var(model.sSlots, model.sPlanes, domain=Binary)
    model.vIdleIn = Var(model.sSlots, model.sPositions, model.sPlanes, domain=Binary)
    model.vEarlySlack = Var(model.sJobs, within=NonNegativeReals)
    model.vLateSlack = Var(model.sJobs, within=NonNegativeReals)
    ...
    def fc02_SlotJobDuration(model, s, p):
        # La duración del slot es la duración del job que está ahí (si hay uno)
        return model.vDurationSlot[s, p] == sum(
            model.pJobDuration[j] * model.v01JobInSlot[s, p, j] for j in model.sJobs)
    ...
    def fc05_JobDuration(model, j):
        return model.vFinishJob[j] == model.vStartJob[j] + model.pJobDuration[j]
...
# Constraits 6 and 7 formulate with Big-M instead of sums - v3.0
    ...
    def fc06_StartJob_upper(model, s, p, j):
        return model.vStartJob[j] <= model.vStartSlotForJob[s, p, j] + model.pHorizon * (
                    1 - model.v01JobInSlot[s, p, j])
...
#         return m.vClientDelay[c] >= m.vPlaneDelay[r]
    ...
    # 1) si x[s,p,j]=1, startSlotForJob == startSlot
    def fc11a_minStart(model, s, p, j):
        return model.vStartSlot[s, p] >= model.vStartSlotForJob[s, p, j]

    def fc11b_maxStart(model, s, p, j):
        return model.vStartSlot[s, p] <= model.vStartSlotForJob[s, p, j] \
            + model.pHorizon * (1 - model.v01JobInSlot[s, p, j])

    model.c11a = Constraint(model.sSlots, model.sPositions, model.sJobs, rule=fc11a_minStart)

    model.c11b = Constraint(model.sSlots, model.sPositions, model.sJobs, rule=fc11b_maxStart)

    # 2) análogo para vFinishSlotForJob vs vFinishSlot

    def fc12a_minFinish(model, s, p, j):
        return model.vFinishSlot[s, p] >= model.vFinishSlotForJob[s, p, j]

    def fc12b_maxFinish(model, s, p, j):
        return model.vFinishSlot[s, p] <= model.vFinishSlotForJob[s, p, j] \
            + model.pHorizon * (1 - model.v01JobInSlot[s, p, j])

    model.c12a = Constraint(model.sSlots, model.sPositions, model.sJobs, rule=fc12a_minFinish)

    model.c12b = Constraint(model.sSlots, model.sPositions, model.sJobs, rule=fc12b_maxFinish)

    # Rule: Ec. SlotSequence - Slot sequence within each position
    def fc13_SlotSequence(model, prev_s, s, p):
        used_prev = sum(model.v01JobInSlot[prev_s, p, j] for j in model.sJobs)
        used_cur = sum(model.v01JobInSlot[s, p, j] for j in model.sJobs)
        # sólo si used_prev=used_cur=1 impone el orden; si uno es 0 se relaja
        return model.vStartSlot[s, p] \
            >= model.vFinishSlot[prev_s, p] \
            - model.pHorizon * (2 - used_prev - used_cur)
    ...
    def fc17_DurationIfNotAssigned(model, s, p, j):
        ...
        return model.vFinishSlotForJob[s, p, j] - model.vStartSlotForJob[s, p, j] == model.pJobDuration[j] * model.v01JobInSlot[s, p, j]
    ...
    def fc20c_PresentExactlyOne(model, s, r):
        # Dentro (exactamente una posición) o fuera (idle fuera), pero no ambas
        return sum(model.vPresence[s, p, r] for p in model.sPositions) + model.vIdle[s, r] == 1
    ...
    def idle_in_lb(m, s, p, r):
        return m.vIdleIn[s, p, r] >= m.vPresence[s, p, r] - m.v01PlaneInSlot[s, p, r]

    def idle_in_ub1(m, s, p, r):
        return m.vIdleIn[s, p, r] <= m.vPresence[s, p, r]

    def idle_in_ub2(m, s, p, r):
        return m.vIdleIn[s, p, r] <= 1 - m.v01PlaneInSlot[s, p, r]
    ...
    def link_start_presence_lb(model, s, p, r):
        return model.vStartPresence[s, p, r] >= model.vStartSlot[s, p] - model.M * (1 - model.vPresence[s, p, r])

    model.cLinkStartPresLB = Constraint(model.sSlots, model.sPositions, model.sPlanes, rule=link_start_presence_lb)
    ...
    def fc27_EarlyStart(model, j):
        plane = model.pPlaneOfJob[j]
        # permitimos retrasar el early en vEarlySlack[j]
        return model.vStartJob[j] >= model.pEarlyStartOfPlane[plane] - model.vEarlySlack[j]

    # Rule: c28_LateFinish – Cada trabajo j de avión r debe acabar antes de pLateFinishDeadline[r]
    def fc28_LateFinish(model, j):
        plane = model.pPlaneOfJob[j]
        # permitimos alargar el late en vLateSlack[j]
        return model.vFinishJob[j] <= model.pLateFinishDeadline[plane] + model.vLateSlack[j]
    ...
    def fc29_NoMovements(model):
        return sum(model.v01JobInSlot[s, p, j] for s in model.sSlots for p in model.sPositions for j in model.sJobs) \
                ...
                + sum(model.vClientDelay[c] for c in model.sClients) \
                + sum(model.vIdle[s, r] for r in model.sPlanes for s in model.sSlots)
    ...
    model.c02_SlotJobDuration = Constraint(model.sSlots, model.sPositions, rule=fc02_SlotJobDuration)
    ...
    model.cIdleIn_lb = Constraint(model.sSlots, model.sPositions, model.sPlanes, rule=idle_in_lb)
    model.cIdleIn_ub1 = Constraint(model.sSlots, model.sPositions, model.sPlanes, rule=idle_in_ub1)
    model.cIdleIn_ub2 = Constraint(model.sSlots, model.sPositions, model.sPlanes, rule=idle_in_ub2)
...
def read_excel(file_name, sheet_name):
    ...
    #pHorizon = max(sum(pJobDuration[j] for j in sJobs if pPlaneOfJob[j] == r)for r in sPlanes) * 1.2

    total_dur = sum(pJobDuration[j] for j in sJobs)
    cap = len(sPositions)
    h_by_capacity = math.ceil(total_dur / max(1, cap) * 1.3)  # 30% de colchón

    pHorizon = max(
        h_by_capacity,
        max(sum(pJobDuration[j] for j in sJobs if pPlaneOfJob[j] == r) for r in sPlanes)
    )
...
if __name__ == "__main__":
    ...
    def precheck_windows(instance):
        infeasible = []
        for j in instance.sJobs:
            plane = instance.pPlaneOfJob[j]
            early = value(instance.pEarlyStartOfPlane[plane])
            late = value(instance.pLateFinishDeadline[plane])
            dur = value(instance.pJobDuration[j])
            if early + dur > late:
                infeasible.append((j, early, dur, late))
        if infeasible:
            print("=== Jobs con ventana inviable ===")
            for j, e, d, l in infeasible:
                print(f"Job {j}: early={e}, dur={d}, late={l} → early+dur={e + d} > late")
        else:
            print("Todas las ventanas temporales son consistentes (early+dur <= late")

    def inspect_slot_conflict(instance, slot, position):
        print(f"--- Inspeccionando slot {slot}, posición {position} ---")
        for j in instance.sJobs:
            try:
                assigned = value(instance.v01JobInSlot[slot, position, j])
            except Exception:
                assigned = None
            if assigned is not None and assigned >= 0.5:
                sj = value(instance.vStartSlotForJob[slot, position, j])
                fj = value(instance.vFinishSlotForJob[slot, position, j])
                sj_global = value(instance.vStartJob[j])
                fj_global = value(instance.vFinishJob[j])
                print(
                    f"Job {j} asignado: slot-start={sj}, slot-finish={fj}, global-start={sj_global}, global-finish={fj_global}, dur={data['pJobDuration'][j]}")


    # Ejecutar antes de construir/solucionar
    precheck_windows(instance)
