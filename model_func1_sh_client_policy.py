...
def read_input(xlsx_path: str, case_sheet: str, planning_start=PLANNING_START, planes_sheet: str = "Planes2"):
    ...
    df_planes = pd.read_excel(xlsx_path, sheet_name=planes_sheet)  # Changed from 'case' to 'planes_sheet'
...
def solve_and_report(model, xlsx_path, case_sheet, timelimit=1500, mipgap=GAP):
    ...
    pd.DataFrame(kpis).to_csv("solution_kpis2.csv", index=False)
    print("CSV exportados: solution_by_position.csv, solution_jobs.csv, solution_kpis2.csv")
...
if __name__ == "__main__":
    XLSX = "case_26.xlsx"
    SHEET = "case"
    d = read_input(XLSX, case_sheet="case", planning_start=PLANNING_START)
    model = build_model(d,
