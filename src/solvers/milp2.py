...
def build_model(d,
    ...
    # Modos: "off" | "soft12" | "soft" | "medium" | "hard"
    # -----------------------------
    ...
    # Links fuertes (1.1 y 1.2) entre z y y para apretar la relajación
    if mode in {"soft12","soft","medium","hard"}:
        ...
        # Construye trabajos por cliente (claves = elementos de m.C)
    ...
    # Penalizar cambios de cliente dentro de una posición (requiere m.w)
    if mode in {"medium","hard"}:
        W_CLIENT_SWITCH_LOCAL = 1_000_000.0  # ajustable
        m.client_change = Var(m.J, m.J, m.P, within=Binary)
        m.c_link_change = ConstraintList()
        # pares (j,k) de clientes distintos con j<k
        diff_pairs = []
        for j in m.J:
            ...
            for k in m.J:
                ...
                if cj != ck:
                    diff_pairs.append((j,k))
        for (j,k) in diff_pairs:
            for p in m.P:
    ...
    if mode in {"medium","hard"}:
        obj_terms.append(W_CLIENT_SWITCH_LOCAL * sum(m.client_change[j,k,p] for j in m.J for k in m.J if k>j for p in m.P))
    if mode == "hard":
...
if __name__ == "__main__":
    XLSX  = "scn_few-loose_seed1_P3_pl5.xlsx"
    SHEET = "case"
