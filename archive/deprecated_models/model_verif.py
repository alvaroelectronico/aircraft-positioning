...
def ap_pyomo_model():
    ...
    model.pFirstJobOfPlane = Param(model.sJobs, model.sPlanes, mutable=True)
    model.pLastJobOfPlane = Param(model.sJobs, model.sPlanes, mutable=True)
    model.pPredictedFinishOfPlane = Param(model.sPlanes, within=NonNegativeReals)
    model.pEarlyStartOfPlane = Param(model.sPlanes, within=NonNegativeReals, mutable=False)
    model.pLateFinishDeadline = Param(model.sPlanes, within=NonNegativeReals, mutable=False)
    ...
    # model.vSlackDeadline = Var(model.sPlanes, within=NonNegativeReals)
    ...
    def fc02_SlotJobDuration(model, s, p, j):
        return model.vDurationSlotForJob[s, p, j] * model.v01JobInSlot[s, p, j] == model.vFinishSlotForJob[s, p, j] - model.vStartSlotForJob[s, p, j]
...
# Constraits 6 and 7 formulate with Big-M instead of sums - v3.0
    ...
    def fc09_Plane_delay(model, r):
        # vPlaneDelay[r] = max(0, finish_last_job[r] - deadline[r])
        return model.vPlaneDelay[r] >= sum(
            (model.vFinishJob[j] - model.pLateFinishDeadline[r])
            * model.pLastJobOfPlane[j, r]
    ...
    def fc14_JobSequence(model, j, j2):
        if (j, j2) not in model.sJobSequence:
            return Constraint.Skip
    ...
    def fc28_EarlyStart(model, j, r):

        return model.vStartJob[j] >= model.pEarlyStartOfPlane[r]- model.M*(1- model.pFirstJobOfPlane [j, r])

    # def fc29_LateFinishSlack(model, r, j):
    #     return sum(model.vFinishJob[j] * model.pLastJobOfPlane[j, r]
    #                for j in model.sJobs) \
    #         <= model.pLateFinishDeadline[r] + model.vSlackDeadline[r]
    ...
    print("Generating c28 & c29: EarlyStart and LateFinish")
    # model.c28_EarlyStart = Constraint(model.sJobs, model.sPlanes, rule=fc28_EarlyStart)
    # model.c29_LateFinishSlack = Constraint(model.sJobs, model.sPlanes, rule=fc29_LateFinishSlack)
...
def read_excel(file_name, sheet_jobs, sheet_planes):
    df = pd.read_excel(file_name, sheet_name=sheet_jobs)
    ...
    pPredictedFinishOfPlane = df.groupby('plane')['predicted_finish'].max().to_dict()
    ...
    min_task_by_plane = df.groupby('plane')['task'].min().to_dict()
    dic_pFirstJobOfPlane = {}
    for r in sPlanes:
        df_r = df[df['plane'] == r]
        if not df_r.empty:
            j0 = df_r[df_r['task'] == min_task_by_plane[r]]['job'].iloc[0]
            for j in sJobs:
                dic_pFirstJobOfPlane[(j, r)] = 1 if j == j0 else 0
        else:
            for j in sJobs:
...
#Datos de aviones
    ...
    df_planes = pd.read_excel(file_name, sheet_name=sheet_planes)
    df_planes['plane'] = df_planes['plane'].astype(df['plane'].dtype)
    df_planes = df_planes[df_planes['plane'].isin(sPlanes)]
    pEarlyStartOfPlane = df_planes.set_index('plane')['early_start'].to_dict()
    pLateFinishDeadline = df_planes.set_index('plane')['late_finish_deadline'].to_dict()
    # Asegurar que todos los aviones en sPlanes tienen valores (por si faltara alguno en la hoja)


    for r in sPlanes:
        pEarlyStartOfPlane.setdefault(r, 0)
        # Si no hay deadline, asumimos pPredictedFinishOfPlane[r] como límite
        pLateFinishDeadline.setdefault(r, pPredictedFinishOfPlane.get(r, 0))
...
def create_data(data):
    ...
    pFirstJobOfPlane = data.get('pFirstJobOfPlane', None)
...
def generate_report(df_planes, model_instance, movimientos):
    ...
    # obtenemos ES/LF (en días) y convertimos a fecha
    ...
    df_det['Duración Real (días)'] = (df_det['finish_slot'] - df_det['start_slot']).dt.total_seconds() / 86400
    df_det['Fecha Prevista'] = df_det['job'].map(lambda j: date.today() + timedelta(days=pDate_map.get(j, 0) + pJobDur[j]))
    df_det['Fecha Real'] = df_det['finish_slot'].dt.date

    # Fecha Límite por avión
    # (pLateFinishDeadline está en días desde el mismo reference de START_DATE)
    late_deadlines = {
        r: int(value(model_instance.pLateFinishDeadline[r]))
        for r in model_instance.sPlanes
    }
    df_det['Fecha Límite'] = df_det['plane'].map(lambda p: date.today() + timedelta(days=late_deadlines[int(p)]))
    df_det['Retraso sobre fecha prevista (días)'] = df_det.apply(lambda row: max((row['Fecha Real'] - row['Fecha Prevista']).days, 0),axis=1)

    def estado_emoji(row):
        if row['Fecha Real'] <= row['Fecha Prevista']:
            return "✅"
        elif row['Fecha Real'] <= row['Fecha Límite']:
            return "🟡"
        else:
            return "🔴"

    df_det['Estado'] = df_det.apply(estado_emoji, axis=1)

    def estado_texto(row):
        if row['Fecha Real'] <= row['Fecha Prevista']:
            return "Avión dentro de fecha prevista"
        elif row['Fecha Real'] <= row['Fecha Límite']:
            return "Avión dentro de Late Finish"
        else:
            return "Avión retrasado"

    df_det['Situación']=df_det.apply(estado_texto, axis=1)

    # df_det['⚠'] = df_det['Retraso sobre fecha prevista (días)'].apply(lambda d: '❌' if d > 0 else '✅')

    df_det = df_det[[
        'Estado', 'plane', 'job', 'p',
        'Fecha Prevista', 'Fecha Real',
        'Duración Estimada (días)', 'Duración Real (días)', 'Retraso sobre fecha prevista (días)', 'Situación'
    ]]
    df_det.columns = [
        'Estado', 'Avión', 'Trabajo', 'Posición',
        'Fecha Prevista', 'Fecha Real',
        'Duración Estimada (días)', 'Duración Real (días)', 'Retraso sobre fecha prevista (días)', 'Situación'
    ]
    ...
    print("=" * 80)
    c2r = {c: r for (c, r), v in model_instance.pAirplaneOfClient.items() if v == 1}
    resumen_c = []
    for c in sorted(model_instance.sClients):
        r = c2r.get(c)
        delay = model_instance.vClientDelay[c].value
        deadline = value(model_instance.pLateFinishDeadline[r]) if r in model_instance.pLateFinishDeadline else None
        estado = '✅ Dentro LF' if delay == 0 else f'❌ Fuera LF (+{int(delay)}d; LF={deadline})'
        resumen_c.append({
            'Cliente': c,
            'Retraso fuera del LF del avión (días)': int(delay),
            'Estado': estado
        })
    ...
    # Totales
    total_t = len(df_det)
    total_r = df_det['Retraso sobre fecha prevista (días)'].gt(0).sum()
    total_a = len(df['plane'].unique())
    total_c = len(resumen_c)
    # Clientes dentro y fuera de LF
    n_out = sum(1 for x in resumen_c if 'Fuera LF' in x['Estado'])
    n_in = total_c - n_out

    print(f"📦 {total_t} trabajos procesados")
    print(f"✈️  {total_a} aviones, {total_c} clientes")
    print(f"🔴 {total_r} trabajos con retraso")
    print(f"✈️  Clientes dentro LF: {n_in}, fuera LF: {n_out}")
    print("ℹ️ El retraso de un cliente sólo considera su último trabajo respecto al deadline.")
    print("=" * 80)
...
# —————————————————————————— c01: SingleJobPerSlot ——————————————————————————
    ...
    # —————————————————————————— c28: EarlyStart (nueva fc28) ——————————————————————————
    verification_results['c28_early_start'] = {'passed': True, 'errors': []}
    for r in sPlanes:
        es = pEarlyStartOfPlane.get(r, None)
        if es is None:
            verification_results['c28_early_start']['passed'] = False
            verification_results['c28_early_start']['errors'].append(
                f"EarlyStart no definido para avión {r}"
            )
            continue
        # sólo para el primer job de cada avión
        for j in sJobs:
            if pFirstJobOfPlane.get((j, r), 0) == 1:
                st_j = start_job.get(j, 0.0)
                if st_j + 1e-6 < es:
                    verification_results['c28_early_start']['passed'] = False
                    verification_results['c28_early_start']['errors'].append(
                        f"Avión {r}, primer trabajo {j}: start_job={st_j:.4f} < early_start={es:.4f}"
                    )
...
if __name__ == "__main__":
    ...
    data = read_excel("input_data.xlsx", "case_3b_planes", "Planes")
    ...
    instance = model.create_instance(input_data)
    print(">>> EarlyStart en instancia:")
    for r in instance.sPlanes:
        print(f"  Avión {r} → ES = {value(instance.pEarlyStartOfPlane[r])}")
    ...
    # for r in instance.sPlanes:
    #     fv = value(instance.vFinishJob[next(j for j in instance.sJobs if value(instance.pLastJobOfPlane[j, r]) == 1)])
    #     pd = value(instance.pLateFinishDeadline[r])
    #     pdly = value(instance.vPlaneDelay[r])
    #     print(f"[CHECK] Plane {r}: finish={fv}, deadline={pd}, vPlaneDelay={pdly}")
    ...
    if results.solver.status.value == "ok":
        ...
        print("Check solución encontrada")
        for r in instance.sPlanes:
            print(f"Avión {r}:")

            # — Early Start del avión (parametro)
            es = value(instance.pEarlyStartOfPlane[r])
            print(f"    → Early Start del avión = {es:.1f}")

            # — Localizar primer job según vStartJob
            #   filtramos sólo los jobs de este avión
            jobs_r = [j for j in instance.sJobs if instance.pPlaneOfJob[j] == r]
            if jobs_r:
                j_first = min(jobs_r, key=lambda j: value(instance.vStartJob[j]))
                s_first = value(instance.vStartJob[j_first])
                print(f"    → Primer trabajo: {j_first}")
                print(f"      → Fecha real de inicio = {s_first:.1f}")

            # — Ahora el último trabajo y resto de métricas
            for j in instance.sJobs:
                if (j, r) in instance.pLastJobOfPlane and value(instance.pLastJobOfPlane[j, r]) == 1:
                    ...
                    print(f"    → Retraso fuera del LF del avión = {instance.vPlaneDelay[r].value:.1f}")
