...
# CONFIG
# -----------------------
PLANNING_START = None  # None => hoy; o "YYYY-MM-DD"

CLIENT_POS_POLICY = "soft"
W_CLIENT_POS   = 50000000.0
...
def read_input(xlsx_path: str, case_sheet: str, planning_start=PLANNING_START):
    ...
    # task puede faltar; si falta, pongo 1
    if 'task' not in df_case.columns:
        df_case['task'] = 1
    df_case['task'] = pd.to_numeric(df_case['task'], errors='coerce').fillna(1).astype(int)

    # late por defecto (o desde otras hojas si existen) + ventanas por avión
    late_map = {}
    ES_plane_map = {}
    try:
        ...
        if 'Planes2' in xls.sheet_names:
            ...
            if 'plane' in _df and 'late_finish' in _df:
                for _, r in _df[['plane','late_finish']].dropna().iterrows():
                    late_map[int(r['plane'])] = float(r['late_finish'])
            if 'plane' in _df and 'early_start' in _df:
                for _, r in _df[['plane','early_start']].dropna().iterrows():
                    ES_plane_map[int(r['plane'])] = float(r['early_start'])
        if ('Planes' in xls.sheet_names):
    ...
    # late por job (si no hay late por avión)
    df_case['late'] = df_case.apply(
        lambda r: late_map.get(int(r['plane']), float(r['early'])+float(r['duration'])+big_pad),
        axis=1
    )
    ...
    df_case = df_case.sort_values(['plane','task','early']).reset_index(drop=True)

    # Jobs (permite varios por avión)
    ...
    # Ventanas por avión (si faltan, neutralizo con 0/H)
    H_jobs = float(max(lf.values()) + max(dur.values()) + 10.0)
    ES_plane = {r: ES_plane_map.get(r, 0.0) for r in planes_set}
    LF_plane = {r: late_map.get(r, H_jobs)  for r in planes_set}

    base_date = pd.Timestamp(date.today()) if planning_start is None else pd.to_datetime(planning_start)
    ...
    # Precedencias por avión (según task)
    preds = []
    for r in planes_set:
        jobs_r = [j for j in jobs if plane_of[j] == r]
        jobs_r.sort(key=lambda j: tasks[j])
        for i in range(len(jobs_r)-1):
            j1, j2 = jobs_r[i], jobs_r[i+1]
            if tasks[j1] < tasks[j2]:
                preds.append((j1, j2))

    # Último trabajo por avión (mayor task)
    last = {}
    for r in planes_set:
        jobs_r = [j for j in jobs if plane_of[j] == r]
        if not jobs_r:
    ...
    data = {
        ...
        'ES_plane': ES_plane, 'LF_plane': LF_plane,
...
def build_model(d,
    ...
    # -----------------------------
    # BLOQUEO (entrada/salida) estilo cpaste.py
    # -----------------------------
    chain = {}
    ...
    if chain:
        ...
        def _gating(mdl, j, k, pb, pf):
            return 2 - mdl.y[j, pb] - mdl.y[k, pf]
