...
def count_model_sizes(m) -> Dict[str, Optional[int]]:
    ...
    """
    try:
        n_constraints = sum(1 for _ in m.component_data_objects(pyo.Constraint, active=True, descend_into=True))
    except Exception:
        n_constraints = None
...
def build_and_solve(model_mod,
    ...
    # 2.1) Tamaño del modelo (variables / restricciones)
    size_info = count_model_sizes(m)
    ...
    metrics = {
        ...
        "n_constraints": size_info.get("n_constraints"),
        "n_vars": size_info.get("n_vars"),
        "n_bin": size_info.get("n_bin"),
        "n_int": size_info.get("n_int"),
        "n_cont": size_info.get("n_cont"),
            ...
            for mode in args.modes:
                for preset in args.solver_presets:
                    ...
                    try:
                        ...
                        print(f"[{fp.name} | {mode} | {preset}] "
                              f"term={metrics.get('termination')} "
                              f"cons={metrics.get('n_constraints')} vars={metrics.get('n_vars')} "
                              f"log={log_file}")
